...
class WatershedWidget(QWidget):
    ...
    def help_button_state(self):
        ...
        alert.setText('Left click: add a seed \n'
    ...
    def label_cell(self, event):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            self.scene.addEllipse(event.pos().x(), event.pos().y(), 10, 10, self.pen, self.brush)
        # delete label on right click
        if event.button() == Qt.MouseButton.RightButton:
            # get list of items at that location
            items = self.scene.items(event.pos().x(), event.pos().y(), 1, 1, Qt.ItemSelectionMode.IntersectsItemShape,
    ...
    def get_coords(self):
        ...
        warnings.warn('Not accurate - needs to be adjusted before exact coordinate is returned')
...
def watershed_segment(img, file_name='Image', coords = None) -> np.ndarray:
    """ Perform watershed segmentation on image - with option either to provide coordinates of markers (coords)
        ...
        Args:
            ...
            plot (bool): Whether to plot the result to the screen
        Returns:
    ...
    for index, coord in enumerate(coords):
        markers[coord[0] - 3:coord[0] + 3, coord[1] - 3:coord[1] + 3] = int(index + 1)
    # perform watershed
