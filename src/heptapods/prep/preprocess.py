...
This module contains functions to preprocess the data, including (add as added):
...
def csv_to_datastruc(input_file, dim, channel_col, frame_col, x_col, y_col, z_col, channel_choice = None):
    """Loads in .csv and converts to the required datastructure.
    Currently considers the following columns:
        channel frame x y z  
    Also user can specify the channels they want to consider, these should be present in the channels column
    ...
    if dim == 2:
        ...
        df = df.rename({channel_col: "channel", frame_col: "frame", x_col: "x", y_col: "y"})
    elif dim == 3:
        df = pl.read_csv(input_file, columns=[channel_col, frame_col, x_col, y_col, z_col])
        df = df.rename({channel_col: "channel", frame_col: "frame", x_col: "x", y_col: "y", z_col: "z"})
    ...
    name = os.path.basename(os.path.normpath(input_file))
